...
public partial class BorrowingTransactionServiceTests
{
    ...
    public void BorrowBook_WhenNoDurationIsSpecified_ShouldUseDefaultDuration()
    {
        ...
        Assert.Equal(borrowingTransaction.TransactionDate, foundBorrowingTransaction.TransactionDate);
        Assert.Equal(borrowingTransaction.DueDate, foundBorrowingTransaction.DueDate);
        Assert.Equal(EXPECTED_BORROWING_DURATION_IN_DAYS, (foundBorrowingTransaction.DueDate - foundBorrowingTransaction.TransactionDate).Days);
    ...
    public void BorrowBook_WhenBookIsUnavailable_ShouldThrowArgumentError() {
        ...
        Assert.Throws<BookUnavailableException>(() => _borrowingTransactionService.BorrowBook(book.Id, member.Id));
    }
    ...
    [Fact]
    public void BorrowBook_WhenDurationIsInvalid_ShouldThrowArithmeticError() {
        // Arrange
        string randomEmail = $"{Guid.NewGuid()}@example.com";
        Member member = Helpers.CreateMember(email: randomEmail);
        Book book = _bookService.AddBook(Helpers.CreateBook());
        ...
        Assert.Throws<ArithmeticException>(() => _borrowingTransactionService.BorrowBook(book.Id, member.Id, duration_in_days));
    }
}
